...
var replaceLinks = function (text, data) {
	if (!text) return "";
...
module.exports = function(data, config, getCurrent){
		...
		apiSection: function(options){
			...
			var makeSignatures = function(signatures, defaultDescription, parent){
				...
				signatures.forEach(function(signature){
					...
					txt += "<p>"+replaceLinks(lastDot != 0 ? description.substr(0, lastDot): description, data)+"</p>"
					txt += "</div>"
				})
			}
