		...
		can.Control('Jmvcdoc.Nav',
			...
			/* @Prototype */
			{
				...
				navFor : function (val) {
					...
					// focus is the last item selected in the path
					// it should never be static or prototype object
					while (focus.parents &&
						( !focus.childDocs || 
						  !focus.childDocs.length || 
						  /static|prototype/i.test(focus.type) )) {
						focus = Doc.findOne({name : focus.parents[0]})
					}
					// get all the parents of the focused object
					var path = [focus], 
						curParent = focus;
