steal('can/control','can/view/ejs').then('./demo.ejs',function(){
...
/* @Prototype */
{
	init: function() {
		...
		$iframe.bind("load", function() {
			...
			self.element.find(".demo_content").css({
				"padding": "5px"
			});

			html = this.contentWindow.DEMO_HTML || $body.find("#demo-html").html();
			self.element.find(".html_content").html("<pre><code class=\"html\"></code></pre>").find("code").text($.trim(html)).highlight();
			$body.find("#demo-instructions").hide();
			source = $body.find("#demo-source").html();


			self.element.find(".source_content").html("<pre><code class=\"javascript\"></code></pre>").find("code").text($.trim(source)).highlight();
			...
			if (!source ) {
				...
				self.element.find(".source_content").html("<pre><code class=\"javascript\"></code></pre>").find("code").text($.trim(standbySource)).highlight();
			}
			var run = function(){
				setTimeout(function() {
					height = $body.outerHeight();
					$iframe.height(height + 50);
					self.element.find(".demo_content").height(height + 55);
				}, 200)
			}
