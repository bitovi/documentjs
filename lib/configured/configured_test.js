...
describe("lib/configured", function(){
		...
		rmdir(path.join(__dirname,"test","tmp"), function(e){
			...
			if(e){
				...
				configured.getProject({
					...
					path: __dirname+"/test/tmp/comparify"
				}).then(function(){
					fs.exists(
	...
	it(".getProject is able to get a folder", function(done){
		this.timeout(10000);
		configured.getProject({
			source: __dirname+"/test/example_project",
			path: __dirname+"/test/tmp/example_project"
		}).then(function(){
			fs.exists(
	...
	it(".geneateProject is able to read the documentjs.json without versions and build a site", function(done){
		...
		rmdir(path.join(__dirname,"test","api"), function(e){
			...
			configured.generateProject({path: __dirname+"/test/example_project"}).then(function(){
	...
	it(".geneateProject is able to build something without a documentjs.json", function(done){
		this.timeout(10000);
	...
	it("sites can be on projects", function(done){
		...
		rmdir(path.join(__dirname,"test","tmp","project_sites"), function(e){
			...
			configured.generateProject({path: __dirname+"/test/project_sites"}).then(function(){
				if(fs.existsSync(path.join(__dirname,"test","tmp","project_sites","docs","index.html"))) {
					done();
				} else {
					done(new Error("test/tmp/project_sites/docs/index.html does not exist"));
				}
			},done);
		});
	});
