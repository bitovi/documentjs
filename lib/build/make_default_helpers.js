...
var docsFilename = function (name) {
...
var replaceLinks = function (text, docMap) {
	if (!text) return "";
	var replacer = function (match, content) {
		...
		if (docMap[link]) {
			return '<a href="' + docsFilename(link) + '">' + description + '</a>';
		}
...
/**
 * @add documentjs.build.defaultHelpers
 */
module.exports = function(docMap, config, getCurrent){
	...
	var helpers = {
		// GENERIC HELPERS
		/**
		 * @function documentjs.build.defaultHelpers.equal
		 */
		...
		/**
		 * @function documentjs.build.defaultHelpers.or
		 */
		or: function(){
		...
		/**
		 * @function documentjs.build.defaultHelpers.notEqual
		 */
		...
		// valueData helpers
		/**
		 * @function documentjs.build.defaultHelpers.makeParamsString
		 * @hide
		 * 
		...
		/**
		 * @function documentjs.build.defaultHelpers.makeType
		 * @hide
		 * 
		...
		/**
		 * @function documentjs.build.defaultHelpers.makeTypesString
		 * 
		...
		/**
		 * @function documentjs.build.defaultHelpers.docLinks
		 * 
		 */
		docLinks: function(text){
			return replaceLinks(text, docMap);
		},
		linkTo: function(name, title, attrs){
			...
			if (docMap[name]) {
				...
				return '<a href="' + docsFilename(name) + '" '+attrsArr.join(" ")+'>' + (title || name ) + '</a>';
			} else {
				return title || name || "";
			}
		},
		...
		/**
		 * @function documentjs.build.defaultHelpers.activeAndParents
		 * 
		...
		/**
		 * @function documentjs.build.defaultHelpers.isActive
		 * 
		...
		apiSection: function(options){
			...
			var makeSignatures = function(signatures, defaultDescription, parent){
				...
				signatures.forEach(function(signature){
					...
					txt += "<p>"+replaceLinks(lastDot != 0 ? description.substr(0, lastDot): description, docMap)+"</p>"
					txt += "</div>"
				});
			};
