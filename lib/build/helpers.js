...
/**
 * @function documentjs.build.helpers
 ...
 * 
 * @signature `documentjs.build.helpers(docMap, options, getCurrent)`
 * 
 * Registers helpers 
 * 
 * 
 * @param {Promise} buildTemplatesPromise The result of calling 
 * [documentjs.build.templates]. Building the helpers
	...
	return buildTemplatesPromise.then(function(){
		...
		return fsx.readdir(dir).then(function(files){
			...
			files.filter(function(filename){
				...
				var requirePath = path.relative( __dirname, path.join(dir, filename) );
				
				var makeHelpers = require(requirePath);
				_.extend(helpers, makeHelpers(docMap, options, getCurrent, helpers) );
