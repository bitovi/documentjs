...
function generateAndWatch(options){
	if(!options.watch) {
		...
		var gazeOptions = _.omit(options.glob,"pattern");
		...
		gaze(pattern, gazeOptions, function(err, watcher){
			this.on('all', regenerate);
		});
		
		if(options.templates) {
			gaze(path.join(options.templates,'*.{js,mustache}'), function(err, watcher){
				this.on('all', regenerate);
			});
		}
		if(options['static']) {
			gaze(path.join(options['static'],'**/*'), function(err, watcher){
				this.on('all', regenerate);
			});
		}
