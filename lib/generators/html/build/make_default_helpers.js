...
var replaceLinks = function (text, docMap) {
	...
	var replacer = function (match, content) {
		...
		if (docMap[link]) {
			return '<a href="' + docsFilename(link, config) + '">' + description + '</a>';
		}
...
module.exports = function(docMap, config, getCurrent){
	...
	var helpers = {
		...
		/**
		 * @function documentjs.generators.html.defaultHelpers.urlFromDocConfig
		 * 
		 * Returns a url that is joined from the most parent `documentjs.json`.
		 */
		urlFromDocConfig: function (url) {
			return path.join(getCurrent().docConfigDest,url);
		},
		/**
		...
		urlDownload: function (docObject) {
			if(docObject.download){
				return helpers.urlFromDocConfig(_.template(docObject.download, {version: docObject.version}));
			} else {
				return "";
			}
		},
