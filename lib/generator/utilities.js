...
exports.replaceLinks = function (text, data) {
	var replacer = function (match, content) {
		var parts = content.split(/\s+/g);
		var link = parts[0].replace('::', '.prototype.');
		var description = parts[1] || link;
...
exports.addLinks = function (current, data) {
	current.body = exports.replaceLinks(current.body, data);
	current.description = exports.replaceLinks(current.description, data);
