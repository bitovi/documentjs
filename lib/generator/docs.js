...
module.exports = function (files, options, callback) {
	...
	documentjs(files, function (error, data, search) {
		...
		var rootItem = _.find(tree, function (current) {
			return current.name == configuration.parent || current.title == configuration.parent;
		});
		...
		if(options.debug) {
			fs.writeFileSync(path.join(configuration.out, 'documentjs-debug.log', beautify(JSON.stringify(data))));
		}
